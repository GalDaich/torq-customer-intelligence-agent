...
const firstPartyContextNode: GraphNode<typeof ResearchState> = async (state) => {
  ...
  try {
    ...
    const firstPartyContext = await extraction.invoke(firstPartyMessages(state.company, corpus));
...
export const researchGraph = new StateGraph({ state: ResearchState, output: ResearchOutput })
  .addNode("firstPartyContext", firstPartyContextNode)
  .addNode("recentSignals", recentSignalsNode)
  .addNode("hiringSignals", hiringSignalsNode)
  .addNode("securitySignals", securitySignalsNode)
...
export async function runCompanyResearch(
  ...
  if (!output.report) throw new Error("Research graph completed without a validated report.");
